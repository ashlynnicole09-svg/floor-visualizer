/* Floor Visualizer with pointer events + Auto/AI floor detection */
...
void main() {
  ...
  u.opacity = gl.getUniformLocation(program, "u_opacity");
  u.exposure = gl.getUniformLocation(program, "u_exposure");
  u.contrast = gl.getUniformLocation(program, "u_contrast");
  window._uniforms = u;
...
function computeGeometryAndUV() {
  ...
  const rot=(u,v)=>{const x=u-cx,y=v-cy;return [x*Math.cos(theta)-y*Math.sin(theta)+cx, x*Math.sin(theta)+y*Math.cos(theta)+cy];};
...
function render() {
  ...
  gl.useProgram(program);
  gl.uniform2f(window._uniforms.resolution, glCanvas.width, glCanvas.height);
  gl.uniform1f(window._uniforms.opacity, (parseFloat(opacity.value)||90)/100.0);
  gl.uniform1f(window._uniforms.exposure, (parseFloat(exposure.value)||100)/100.0);
  gl.uniform1f(window._uniforms.contrast, (parseFloat(contrast.value)||100)/100.0);
  const geo = computeGeometryAndUV(); if (!geo) return;
  const a_pos = gl.getAttribLocation(program, "a_pos");
  const a_uv  = gl.getAttribLocation(program, "a_uv");
  gl.bindBuffer(gl.ARRAY_BUFFER, posBuffer); gl.bufferData(gl.ARRAY_BUFFER, geo.pos, gl.DYNAMIC_DRAW);
  gl.vertexAttribPointer(a_pos, 2, gl.FLOAT, false, 0, 0);
  gl.bindBuffer(gl.ARRAY_BUFFER, uvBuffer);  gl.bufferData(gl.ARRAY_BUFFER, geo.uv,  gl.DYNAMIC_DRAW);
  gl.vertexAttribPointer(a_uv,  2, gl.FLOAT, false, 0, 0);
...
function drawGuides() {
  ...
  points.forEach((p,i)=>{
    ...
    c.setAttribute("cx", p.x); c.setAttribute("cy", p.y); c.setAttribute("data-idx", i);
...
function onPointerDown(e){
  ...
  if (idx !== -1) {
    dragging = idx;
    overlay.setPointerCapture && overlay.setPointerCapture(e.pointerId || 0);
    return;
  }
  if (points.length<4) {
    points.push(pt);
    if (points.length===4) render(); else drawGuides();
  }
}
...
function onPointerUp(e){
  if (dragging!=null) {
    dragging = null;
    overlay.releasePointerCapture && overlay.releasePointerCapture(e.pointerId || 0);
  }
}
...
btnResetPoints.addEventListener("click", ()=>{
  points = []; measurePts = null; measureLine.classList.add("hidden"); drawGuides(); render();
});
...
[plankLen, plankWid, angle, opacity, exposure, contrast].forEach(inp=> inp.addEventListener("input", render));
texInput.addEventListener("change", async (e)=>{ const f=e.target.files[0]; if(!f) return; const img=await loadImage(f); setTexture(img); });
btnExportImage.addEventListener("click", ()=>{
  if (!roomImg.src) { alert("Upload a room photo first."); return; }
  const out = document.createElement("canvas");
  out.width = roomImg.naturalWidth; out.height = roomImg.naturalHeight;
...
btnLoadDemo.addEventListener("click", async ()=>{
  ...
  demo.onload = ()=>{ roomImg.src = demo.src; roomImg.onload = ()=>{ setCanvasSizeToImage(); fitImage(); const w=roomImg.naturalWidth,h=roomImg.naturalHeight; points=[{x:w*0.18,y:h*0.72},{x:w*0.82,y:h*0.72},{x:w*0.98,y:h*0.98},{x:w*0.02,y:h*0.98}]; drawGuides(); render();}; };
  const c = document.createElement("canvas"); c.width=1400; c.height=900; const gctx=c.getContext("2d");
  const grad=gctx.createLinearGradient(0,0,0,c.height); grad.addColorStop(0,"#2b3545"); grad.addColorStop(0.5,"#222b38"); grad.addColorStop(1,"#1a2230");
  gctx.fillStyle=grad; gctx.fillRect(0,0,c.width,c.height);
  gctx.fillStyle="#3a4454"; gctx.fillRect(0,0,c.width,c.height*0.6);
...
async function detectFloorWithCV() {
  ...
  const w = roomImg.naturalWidth, h = roomImg.naturalHeight;
  const scale = Math.min(800 / Math.max(w, h), 1);
  const dw = Math.max(1, Math.round(w * scale)), dh = Math.max(1, Math.round(h * scale));
  const c = document.createElement("canvas"); c.width = dw; c.height = dh;
  const ctx = c.getContext("2d"); ctx.drawImage(roomImg, 0, 0, dw, dh);
  ...
  for (let i=0; i<contours.size(); i++) {
    ...
    if (area < (dw*dh*0.05)) { cnt.delete(); continue; }
    const rect = cv.boundingRect(cnt);
    const bottomBias = rect.y + rect.height >= dh*0.6 ? 1 : 0;
    const peri = cv.arcLength(cnt, true);
    const approx = new cv.Mat(); cv.approxPolyDP(cnt, approx, 0.02*peri, true);
    let verts = approx.rows;
    let vertexScore = (verts===4)?2:(verts===5?1.4:(verts===6?1.2:1));
...
function drawInitial(){ initGL(); drawGuides(); }
drawInitial();
